from deplacements import lancer_de


def test_deplacements():
  ...
  'loyer':{'Cafet Bréguet': 5, 'Interlocal': 10, 'RU Eiffel': 15, 'Local ViaRézo': 20, 'Musée': 25, 'Amphi Michelin': 30, 'RU Bréguet': 35}}
    ...
    lancer_de(1, monopoly, joueurs)             
    assert joueurs['position']['1']!=0          # vérifier que le joueur s'est déplacé
    for i in range (8):
        lancer_de(1, monopoly, joueurs)         # joue un nombre de fois qui garantit que le joueur est passé par la case départ
    assert joueurs['argent']['1']>1000          # vérifie que le joueur a perçu de l'argent en passant par la case départ
